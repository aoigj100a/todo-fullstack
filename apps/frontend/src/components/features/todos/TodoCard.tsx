...
export function TodoCard({
  ...
  return (
    <Card
      ...
      data-todo-id={_id}
    >
      <div className={cn('flex items-center gap-4', variant === 'compact' ? 'p-3' : 'p-4')}>
        {/* 狀態圖標 - 左側互動區域 */}
        <div
        ...
        <div className="flex-1 min-w-0 py-2 relative">
          ...
          {variant === 'default' && (
            <Badge className={cn('mt-2 select-none', statusColor[status])}>
        ...
        <div className="flex items-center gap-2 relative z-50 flex-shrink-0">
          ...
          <Button
            ...
            className={cn(
              'transition-all duration-200 text-red-600 hover:text-red-700 relative z-50',
              variant === 'compact'
                ? 'opacity-60 hover:opacity-100' // Board 視圖中始終可見
                : 'opacity-0 group-hover:opacity-100', // List 視圖中 hover 顯示
              isDragging ? 'pointer-events-none' : 'pointer-events-auto',
